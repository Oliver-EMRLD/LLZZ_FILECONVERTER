...
void checkarg( char *arg[], int till , int from , char *arg1 , char *arg2 )
    {
    int debug = -1;
    int res;
    ...
    for(int x = from ; x < till ; x++)
        ...
        res = debug;
        debug = memcmp(arg[x], arg2, strlen(arg2));
        res = res*debug;
        if(res != 0)
            {
...
int strcut(char *buffer, char cutplace)
    {
    int len = strnlen(buffer, 4096);

    for(int x = 0 ; x <= len ; x++)
        {
        if(buffer[x] == cutplace)
            {
            buffer[x] = '\00';
            //printf(", Found\n");
            return 0;
