...
/*
char *pathin = "/home/oliver/tst/output";
char *pathout = "/home/oliver/tst/omg";
char *mode = "FIXED";
char *hex = "YES";
char *browse = "NO";
...
// You need to check if EOL is where it is, and check the length, compare with actual length
    ...
    if(memcmp( HEX , "YES" , 4 ) == 0)
        ...
        for( x = 0 ; x < Arrindex  ; x++ ) //ARRINDEX
            ...
            for( int y = 0 ; y <= Arrreclen[x]-4 ; y++ )
                {
                //printf("\n###########################################################################\n###########################################################################\n");

                upper = *(buffer+(Largestrecordsize+SUFFIX_LENGTH-4)*(x*3+1)+y);
                //printf("Upper = %x\n ", upper);
                //printf("Location : %i\n", (Largestrecordsize+SUFFIX_LENGTH-4)*(x*3+1)+y);

                lower = *(buffer+(Largestrecordsize+SUFFIX_LENGTH-4)*(x*3+2)+y);
                //printf("Lower = %x\n", lower);
                //printf("Location : %i\n", (Largestrecordsize+SUFFIX_LENGTH-4)*(x*3+2)+y);
                hextocharptr = unhex(upper, lower);
                //printf("\nUNHEXRESULT : %c ; %x\n", hextocharptr, hextocharptr);
                output[Arrrecloc[x]+4+y] = hextocharptr;
    ...
    if(memcmp(HEX, "YES", 4) == 0)
        ...
        for(x = 0 ; x < rec_len_index ; x++)
            ...
            for( HexInd = 0 ; HexInd < rec_len[x]-4 ; HexInd++)
                {
                chartohex(*(buffer_for_file+4+rec_loc[x]+HexInd),  &LineInHex);
